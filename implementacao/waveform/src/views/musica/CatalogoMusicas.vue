...
async function fetchUserPlaylists() {
  try {
    ...
    // Buscando detalhes para o filtro funcionar
...
<template>
  <div class="container mt-4">
    ...
    <div v-if="loading" class="text-center my-5"><div class="spinner-border text-primary"></div></div>
    ...
    <div v-else>
      <div v-for="song in audiosFiltrados" :key="song.id" class="card p-3 mb-2 d-flex flex-row justify-content-between align-items-center">
        ...
        <div class="d-flex gap-2">
          <button class="btn btn-sm btn-outline-secondary" @click="download(song)"><i class="fa-solid fa-download"></i></button>
          <button class="btn btn-sm btn-outline-secondary" @click="abrirModalPlaylist(song)"><i class="fa-solid fa-plus"></i></button>
    ...
    <div v-if="showPlaylistModal" class="modal-overlay">
      <div class="modal-content card p-4 shadow-lg border-primary">
        ...
        <div class="modal-body custom-scroll">
          <div class="d-flex flex-column gap-2"> <!-- GAP 2 PARA ESPAÇO ENTRE BOTÕES -->
            <button 
              v-for="p in playlistsParaAdicionar" 
              :key="p.id"
...
.modal-overlay {
  ...
  background: rgba(0,0,0,0.85); display: flex; justify-content: center; align-items: center; z-index: 1050;
}
...
.playlist-btn {
  ...
  border: 1px solid var(--border) !important; /* Borda leve em volta de cada um */
  padding: 12px;
  font-weight: bold;
